...
def paper_plot_labels(ax, rule_nicename, labels, ref_fontsize = 14):
    ...
    ax.set_title(f'{rule_nicename}', fontsize=title_fontsize, pad=pad)
...
def print_game_plot(region, name, instance, profile, _costs, _last_costs,
                    ...
                    ref_fontsize = 14, limit=1., ax=None):
    ...
    if not fig:
        ...
        paper_plot_labels(ax, rule_nicename, labels, ref_fontsize)
    else:
...
def print_game_plot_with_none(region, name, instance, profile, _costs, _last_costs,
                              _original_winners, _winners, rule, lower_bound, r,
                              ref_fontsize = 14, limit=1., ax = None):
    fig = None
    if not ax:
      fig, ax = plt.subplots(figsize=(8, 4))
    ...
    if not fig:
        ...
        name = name.replace('.pb', '')
        lower_bound = str(lower_bound).replace('.', '')
        plt.savefig(f'images/games/{region}/{name}_{rule}_{lower_bound}_{r}', dpi=200, bbox_inches='tight')
        #plt.savefig(f'images/games/{region}/{name}_{rule}_{r}', dpi=200, bbox_inches='tight')
        plt.close()
...
def plot_to_cr_neurips():
    ...
    fig.text(left_beg_of_first_label, legend_y_pos + 0.015,
             'Originally winning project', fontsize=legend_fontsize,
             va='center', ha='left')
    ...
    eq_marker_legend_xpos =  left_beg_of_first_label + 0.1625
    ...
    fig.text(eq_marker_legend_xpos + 2.5*dist_from_patch, legend_y_pos + 0.015,
             "Equilibrium cost (if computable)",
             fontsize=legend_fontsize , va='center', ha='left')

    legend_rect = Rectangle(
    (0.06, -0.01), 0.36, 0.055,              
    ...
    fig.savefig(f'images/games_cr', dpi=300, bbox_inches='tight')
